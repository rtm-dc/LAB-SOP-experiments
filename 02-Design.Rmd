...
There are `r sum(x100$g == "a")` units in group `a`, `r sum(x100$g == "b")` in `b`, and `r sum(x100$g == "c")` in `c`. Below, see that the assignment within blocks is perfectly half 1's, half 2's. Similarly, `b1` and `b2` will be better balanced on average than in an unblocked assignment. See the resources above for details.
...
outcomes, we need `r ceiling(power_out$n)` observations in each group to have a
probability of `r power_out$power` of detecting a true effect that is one
...
We see a minimum detectable effect of about `r round(power_out_mde$p2 - power_out_mde$p1, 2)`, over a base rate of 0.5.
...
So the probability of detecting the true average treatment effect of `r true_te` is about `r mean(is_stat_sig)`. This high power comes largely from the strongly predictive nature of the covariate `x`. Note that a naïve formula-based approach that ignores the data generating process estimates the power to be roughly `r round(power.t.test(n = 50, delta = 1, sd = 2.7)$power, 3)`.
