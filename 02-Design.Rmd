...
For two equally-sized samples drawn from a population with standard normal outcomes, we need `r ceiling(power_out$n)` observations in each group to have a probability of `r power_out$power` of detecting a true effect that is one standard deviation of the outcome in size, where "detecting" means rejecting a null hypothesis of $H_0: \mu_{Tr} = \mu_{Co}$ against an alternative of $H_a: \mu_{Tr} \neq \mu_{Co}$ using $\alpha = 0.05$.
...
So the probability of detecting the true average treatment effect of `r true_te` is about `r mean(is_stat_sig)`. This is high power comes largely from the strongly predictive nature of the covariate `x`. Note that a naïve formula-based approach that ignores the data generating process estimates the power to be roughly `r round(power.t.test(n = 50, delta = 1, sd = 2.7)$power, 3)`.
